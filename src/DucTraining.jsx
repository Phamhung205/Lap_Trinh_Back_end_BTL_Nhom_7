...
const DucTraining = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 font-sans text-slate-800 p-4 sm:p-6 lg:p-8 bg-slate-50 min-h-screen overflow-x-hidden">
      
      {/* HEADER CỦA ĐỨC */}
      <header className="mb-6 sm:mb-8">
        <h2 className="text-2xl sm:text-3xl font-black text-slate-800 tracking-tight">Đào Tạo & Kết Quả</h2>
        <p className="text-sm sm:text-base text-slate-500 font-medium mt-1">Quản lý khóa học lái xe và xét duyệt kết quả thi thực hành.</p>
      </header>

      <div className="bg-purple-50 text-purple-700 p-4 rounded-2xl font-bold flex items-start sm:items-center gap-3 text-sm sm:text-base">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <form onSubmit={handleAddCourse} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                ...
                <input type="number" value={newCourse.price} onChange={e => setNewCourse({...newCourse, price: Number(e.target.value) || 0})} className="w-full bg-slate-50 p-3 rounded-xl outline-none font-bold" />
              </div>
              <div>
                <label className="text-[10px] font-bold text-slate-400">GIỜ HỌC</label>
                <input type="number" value={newCourse.durationHours} onChange={e => setNewCourse({...newCourse, durationHours: Number(e.target.value) || 0})} className="w-full bg-slate-50 p-3 rounded-xl outline-none font-bold" />
              </div>
              <div>
                <label className="text-[10px] font-bold text-slate-400">KM YÊU CẦU</label>
                <input type="number" value={newCourse.requiredKm} onChange={e => setNewCourse({...newCourse, requiredKm: Number(e.target.value) || 0})} className="w-full bg-slate-50 p-3 rounded-xl outline-none font-bold" />
              </div>
            </div>
        ...
        <div className="bg-slate-900 p-5 sm:p-6 rounded-3xl shadow-lg shadow-slate-900/20 text-white relative overflow-hidden flex flex-col justify-between">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 mb-6 w-full sm:w-3/4">
          ...
          <div className="relative z-10">
            <form onSubmit={handleSubmitScore} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
              <input type="number" value={scoreForm.studentId} onChange={e => setScoreForm({...scoreForm, studentId: e.target.value})} placeholder="ID Học viên..." className="w-full bg-slate-800 p-4 rounded-xl outline-none focus:ring-2 focus:ring-yellow-500 font-bold" />
              <input type="number" value={scoreForm.score} onChange={e => setScoreForm({...scoreForm, score: e.target.value})} placeholder="Điểm (0-100)..." className="w-full bg-slate-800 p-4 rounded-xl outline-none focus:ring-2 focus:ring-yellow-500 font-bold" />
              <button type="submit" className="w-full sm:col-span-2 lg:col-span-1 bg-yellow-500 text-slate-900 font-black p-4 rounded-xl hover:bg-yellow-400 transition shadow-lg shadow-yellow-500/20">
                XÉT DUYỆT
              </button>
            </form>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {courses.length > 0 ? courses.map(c => (
            <div key={c.id} className="bg-slate-50 p-6 rounded-2xl hover:bg-purple-50 transition border border-transparent hover:border-purple-100 group">
              <h4 className="text-lg sm:text-xl font-black text-slate-800 mb-2 break-words group-hover:text-purple-700 transition-colors">{c.name}</h4>
              <div className="flex items-center gap-2 text-purple-600 font-black text-xl mb-4">
                {(c.price || 0).toLocaleString()} VNĐ
              </div>
              <div className="flex flex-wrap gap-3 text-[11px] font-black text-slate-400 uppercase">
